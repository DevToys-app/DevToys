...
{
    private static readonly Dictionary<string, char> characterEntities
        = new()
        {
    ...
    internal static char? Decode(string entity)
    {
        ...
        {
            return DecodeNumeric(entity.Substring(3), @base: 16);
        }
        
        if (entity.StartsWith("&#", StringComparison.Ordinal))
        {
            return DecodeNumeric(entity.Substring(2), @base: 10);
        }

        if (characterEntities.TryGetValue(entity, out char value))
        {
            return value;
        }
    ...
    private static char DecodeNumeric(string input, int @base)
    {
        uint code = Convert.ToUInt32(input, @base);
