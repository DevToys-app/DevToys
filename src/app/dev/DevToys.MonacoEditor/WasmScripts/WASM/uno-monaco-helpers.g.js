...
const asyncCallback = (promiseId, parameter) => {
...
const invokeWithReturnValue = (methodToInvoke) => {
...
const getParentValue = (editorContext, name) => {
...
const getParentJsonValue = function (editorContext, name) {
    return new Promise((resolve, reject) => {
        const json = invokeWithReturnValue((returnId) => editorContext.Accessor.getJsonValue(name, returnId));
        resolve(json);
    });
};
const callParentEventAsync = (editorContext, name, parameters) => invokeAsyncMethod((promiseId) => __awaiter(this, void 0, void 0, function* () {
    let result = yield editorContext.Accessor.callEvent(name, promiseId, parameters != null && parameters.length > 0 ? stringifyForMarshalling(parameters[0]) : null, parameters != null && parameters.length > 1 ? stringifyForMarshalling(parameters[1]) : null);
    if (result) {
...
const callParentActionWithParameters = (name, parameters) => Parent.callActionWithParameters(name, parameters != null && parameters.length > 0 ? stringifyForMarshalling(parameters[0]) : null, parameters != null && parameters.length > 1 ? stringifyForMarshalling(parameters[1]) : null);
